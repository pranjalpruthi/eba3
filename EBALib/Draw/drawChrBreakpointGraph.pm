# $Id$ Draw Chromosome Breakpoint Graph
# Perl module for EBA EBALib::Draw::drawChrBreakpointGraph;
...
EBALib::Draw::drawChrBreakpointGraph  - DESCRIPTION of Object
...
package EBALib::Draw::drawChrBreakpointGraph;

#use Term::ANSIColor;

use Exporter;

our @EXPORT_OK = "breakpointChrGraph";

sub breakpointChrGraph {
my ($fileName, $spsArray_ref, $allResolutions_ref)=@_;
my @spsArr=@$spsArray_ref;
my @allRes=@$allResolutions_ref;
my $file2="drawChrBreakpointGraph.data";
...
my @spsArray = sort { $a cmp $b } @spsArr;
	...
	foreach my $speciesName(@spsArray) {    # print "$speciesName\n";
			...
			open INFILE,  "$fileName" or die "$0: open $fileName : $!";
				while (<INFILE>) {
					...
					#next if $tmp[3] ne $speciesName;
						if (("$tmp[2]" eq "$chr") and ($tmp[3] eq $speciesName) and ($tmp[6] eq "break")) { ## Currently break is lower case
		...
		my $newcountBreakNumber=$countBreakNumber/scalar(@allResolutions);
		my $newNum=$newcountBreakNumber/$hash{$chr};
...
#-generateGraph (\@finalData,\@catColor, \@spsArray, $maximum);
...
my ($finalData_ref, $catColor_ref,$allSps_ref, $max)=@_;
...
q^
my $my_graph = new GD::Graph::linespoints(1000,600,1);
#set graph options required


$my_graph->set(
	'title' => 'EBRs in all resolutions', #graph title
	'y_label' => 'Breakpoints_Rates', #y-axis label
	'x_label' => 'Reference_Chromosomes',
...
my $line_file = "breakChrGraph.gif";
...
print "Created a reference chromosome based EBRs line graph in $currentdir/$line_file\n";
