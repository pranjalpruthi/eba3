# $Id$ Draw Cumulative Stack Bar
# Perl module for EBA EBALib::Draw::drawCumulatedStackedBar;
...
EBALib::Draw::drawCumulatedStackedBar  - DESCRIPTION of Object
...
package EBALib::Draw::drawCumulatedStackedBar;
...
our @EXPORT_OK = "drawStackBar";

sub drawStackBar {
my ($path, $resName, $num) = @_;
my $dir="$path/EBA_OutFiles";
#print "Generating cumulative bar chart for breaks and pseudobreaks.....\n";
		 ...
		 opendir(DIR, $dir) or die $!;
			...
			my @new_file=split(/_/, $file);
				if ($new_file[0] ne "") { 
					...
					my $OutPutFile= "$path/EBA_ImageFiles/Number_unique_reuse_EBRs_$resName.gif";
					my $OutPutFile2= "$path/EBA_ImageFiles/Number_unique_reuse_EBRs_$resName.data";
					my @resN=split(/\//, $path);
					drawStackBarChart($InPutFile, $OutPutFile, $OutPutFile2, $resN[-1], $num);
...
my ($InPutFile, $OutPutFile, $OutPutFile2, $resN, $num)=@_;

#$InPutFile="final_classify_reuse.eba8";
#$OutPutFile="StackBarGraphFinal.gif";
#$OutPutFile2="StackBarGraphFinal.data";

my ($finalData_ref, $catColor_ref, $allSps_ref)=readData($InPutFile, $OutPutFile2, $resN);
...
my $firstN=(900+($num*5)); my $secondN=(800+($num*5));
...
$my_graph->set( 
	'title' => "No. unique and reuse EBRs per species and phylogenetic group ($resN Kbp)", #graph title
	'y_label' => 'No. EBRs', #y-axis label
	#'x_label' => 'Classification_Groups',
...
print "\nCreated a cumulated stacked bar graph for $resN Kb EBRs in $OutPutFile";
...
close OUTFILE2 or die "could not close file: $!\n";
}
