# $Id$ Draw Breakpoint Final Graph2
# Perl module for EBA EBALib::Draw::drawBreakpointChrGraphFinal2;
...
EBALib::Draw::drawBreakpointChrGraphFinal2  - DESCRIPTION of Object
...
package EBALib::Draw::drawBreakpointChrGraphFinal2;
...
my $file2=EBALib::CommonSubs::outpath("EBR_density_chromosomes_merged.data");
...
my (@allBreaks, $countBreakNumber, @finalData, @all, @catColor, @arrayChr, @arrayChr2, @newspsArray, @spsArray2);
...
#print "$hash{$_}\t$_\n";
	my $chrSizeInMB=$hash{$_}/1000000;
	my $newNum=nearest(.01, $chrSizeInMB); 
	push (@arrayChr2, $newNum); 
	push (@arrayChr, $_);
	} ## Store the chromosome data


my @spsArray = sort { $a cmp $b } @spsArr;
@newspsArray=@spsArray; # No need to declare it again
foreach my $aaa (@newspsArray) {$aaa=~ s/_/ /ig; push (@spsArray2, $aaa);}

	push (@finalData, \@arrayChr2);
	foreach my $speciesName(@spsArray) {    # print "$speciesName\n";
		...
		my $newScore=$countBreakNumber/$chrSizeInMB;	
		if(!$countBreakNumber) { $newNum=0; } else { $newNum=stround($newScore,2);} ## stround fuction 

		#if (!$countBreakNumber) {$countBreakNumber=1;}
		#my $newNum=$hash{$chr}/$countBreakNumber;
		push (@allBreaks, $newNum);
		push (@all, $newNum);
		$countBreakNumber=0;
		}
	push (@allBreaks, $speciesName); ## To add the name of species
...
#-generateGraph (\@finalData,\@catColor, \@spsArray2, $maximum, $resName, $spsNum);
}
...
my ($finalData_ref, $catColor_ref,$allSps_ref, $max, $resName, $spsNum)=@_;
...
my $firstN=(1600+($spsNum*2)); my $secondN=(800+($spsNum*2));
q^
my $my_graph = new GD::Graph::linespoints($firstN,$secondN,1);
#set graph options required

$my_graph->set(
	'title' => 'Density of EBRs per chromosome size at merge resolution', #graph title
	'y_label' => 'No. EBRs per Mbp', #y-axis label
	'x_label' => 'Reference chromosome size (Mbp)',
	...
	#'markers' => [ 1, 5 ],
	'skip_undef' => 1,
	'line_types' => [ 3, 4 ],  ## Available line types are 1: solid, 2: dashed, 3: dotted, 4: dot-dashed. default is 1 (solid) [ 4, 3, 2, 1 ]
	#'borderclrs'=> 'black',
...
my $line_file = EBALib::CommonSubs::outpath("EBR_density_chromosomes_merge.gif");
...
print "Created a reference chromosome based line graph for EBRs in $currentdir/$line_file\n";
