# $Id$ Reuse Merge
# Perl module for EBA EBALib::FindReuseMerge;
...
EBALib::FindReuseMerge  - DESCRIPTION of Object
...
our @EXPORT_OK = "generateReuseMerge";

sub generateReuseMerge {
...
print OUTFILE "Reference genomme name\tReference Chr.\tFinal classification\tNarrowest  EBR interval start (bp)\tNarrowest  EBR interval end (bp)\tClass\tScores\tRatio between the first and second classification probabilities\tNo. species in the dataset\tNo. species with gap\tNo. species with the EBR\tPercentage informative species\tRatio between the first and second classification probabilities adjusted for reuse EBRs\n";

$|++;
$/ = "\n";

while (<INFILE>) {
	my $line=lc($_); chomp $line; $line=EBALib::CommonSubs::trim($line); ## lowercase
	if ($line =~ /^\s*#/) { next; }
	...
	my @coordinates= split /\<\-\>/, $result[3]; ## Why did the number is differerent here ???
	...
	foreach (@allClassGroup) { my @GroupValues=split /\:/, $_;  $allClassGroupHash{$GroupValues[0]}=$GroupValues[1]; } ## store all the classification name and its scores.
		...
		if ((($result[8] <= $cutoff) and ($result[8] > 1)) and (scalar(@intersection) == 0)) { 
			...
			checkReuse(\@coordinates, \@classNamePahala, \@classNameDusara, \@result, \@allClassGroup, \%allClassGroupHash, \%allClassificationHash, $percentageUsed, $cutoff, 1); ## flag 1 if non-overlapping means REUSE
			
			}
		elsif ($result[8] == 1) {
			my $newroundRatio1;
			if(($result[8]) ne "na") { $newroundRatio1=sprintf("%.2f", $result[8]);} else { $newroundRatio1="na";}
			my $newperUsed1=round($percentageUsed);	
			print OUTFILE "Reference_Name\t$result[0]\t$classNamePahala[0]:$classNameDusara[0]\t$coordinates[0]\t$coordinates[1]\tUncertain\t$classNamePahala[1]:$classNameDusara[1]\t$result[8]\t$result[10]\t$result[11]\t$result[12]\t$newperUsed1\t$newroundRatio1\n";
			}
		else {
			my $signal=checkReuse(\@coordinates, \@classNamePahala, \@classNameDusara, \@result, \@allClassGroup, \%allClassGroupHash, \%allClassificationHash, $percentageUsed, $cutoff, 0); ## Overlapping
			if ($signal == 0) {
				my $newroundRatio;
				if(($result[8]) ne "na") { $newroundRatio=sprintf("%.2f", $result[8]);} else { $newroundRatio="na";}
				my $newperUsed2=round($percentageUsed);			
				print OUTFILE "Reference_Name\t$result[0]\t$classNamePahala[0]\t$coordinates[0]\t$coordinates[1]\tUnique\t$classNamePahala[1]\t$result[8]\t$result[10]\t$result[11]\t$result[12]\t$newperUsed2\t$newroundRatio\n";
				}
...
my ($coordinates_ref,$classNamePahala_ref, $classNameDusara_ref, $result_ref, $allClassGroup_ref, $allClassGroupHash_ref, $allClassificationHash_ref, $percentageUsed, $cutoff, $flag)=@_;
...
#my $nextMaximum=0;
	if($flag == 1) { ## if it reuse		
		...
		for my $num (0 .. $#storeName)  { 
			...
			#if ($diffValue < 0) { $newScoreReuse=$sumRatio/scalar(@storeRatio);} else { $newScoreReuse=$averageScore/$nextMaximum;}

			print OUTFILE "Reference_Name\t$result[0]\t$myClassNameMain\t$coordinates[0]\t$coordinates[1]\tReuse\t$myClassScoreMain\t$myClassRatioMain\t$result[10]\t$result[11]\t$result[12]\t$percentageUsed\t$newScoreReuse\n"; 
...
sub generateFinal {
	...
	if ($fnum==1){
		...
		if ($. == 1) { print FILE2 "Chromosome\tWide_Breakpoint\tScore\tNarrow_Breakpoint_Start\tNarrow_Breakpoint_End\tSpecies\tClassGroup\tFirst_Probability\tSecond_Probability\tRatio\tAll_Species\tGaps\tBreakpoints\tPercentage_Used\n";  next;}
		...
		print FILE2 "$tmpLine[$spsNum+1]\t$newWide[0]\t$newWide[1]\t$tmpLine[$spsNum+3]\t$narrowCoordi[0]\t$narrowCoordi[1]\t$tmpLine[$spsNum+5]\t$tmpLine[$spsNum+6]\t$tmpLine[$spsNum+7]\t$tmpLine[$spsNum+8]\t$roundRatio\t$tmpLine[$spsNum+11]\t$tmpLine[$spsNum+12]\t$tmpLine[$spsNum+13]\t$newperUsed\t$pval\n";
		
	}
	#elsif($fnum==2){
		#print FILE2 "$tmpLine[0]\t$tmpLine[1]\t$tmpLine[2]\t$tmpLine[3]\t$tmpLine[4]\t$tmpLine[5]\t$tmpLine[6]\t$tmpLine[7]\t$tmpLine[8]\t$tmpLine[9]\t$tmpLine[10]\t$tmpLine[11]\t$tmpLine[12]\n";
	#}
    }
    close FILE1;
    close FILE2;
...
sub generateFinal2 {
	...
	if (($fnum==1) and ($engrave == 0)){
		...
		if ($. == 1) { print FILE2 "Chromosome\tWidest EBR interval (bp)\t\t\tNarrowest  EBR interval start (bp)\tNarrowest  EBR interval end (bp)\tSpecies containing the EBR\tClassification group:assignment score\tHighest probability classification\tSecond highest probability classification\tRatio between the first and second probabilities\tNo. species in the dataset\tNo. species with gap\tNo. species with the EBR\tPercentage informative species\tError probability\n";  next;}
		my @narrowCoordi=split(/\<\-\>/, $tmpLine[$spsNum+4]);
		my $perUsed=(($tmpLine[$spsNum+11]-$tmpLine[$spsNum+12])/$tmpLine[$spsNum+11])*100;
		my $roundRatio;
		if(($tmpLine[$spsNum+9]) ne "NA") { $roundRatio=sprintf("%.2f", $tmpLine[$spsNum+9]);} else { $roundRatio="NA";}
